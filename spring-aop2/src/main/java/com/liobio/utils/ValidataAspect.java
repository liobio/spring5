...
public class ValidataAspect {
    @Before("com.liobio.utils.LogUtils.OniMyPoint()")
    public static void logStart(JoinPoint joinPoint) {
        String name = joinPoint.getSignature().getName();
        Object[] args = joinPoint.getArgs();
        System.out.println("【ValidataAspect】=====【"+name+"】方法开始执行了，参数为【"+ Arrays.asList(args) +"】");
    }
    @AfterReturning(value = "com.liobio.utils.LogUtils.OniMyPoint()",returning = "result")
    public static void logReturn(JoinPoint joinPoint,Object result) {
        String name = joinPoint.getSignature().getName();
        System.out.println("【ValidataAspect】=====【"+ name +"】方法执行完成，他的结果为是："+result);
    }
    @AfterThrowing(value = "com.liobio.utils.LogUtils.OniMyPoint()",throwing = "e")
    public static void logException(JoinPoint joinPoint,Exception e) {
        String name = joinPoint.getSignature().getName();
        System.out.println("【ValidataAspect】=====【"+ name +"】方法出现了异常，异常为："+ e);
    }
    @After("com.liobio.utils.LogUtils.OniMyPoint()")
